...
/**
 * All lexer methods that used in grammar (IsStrictMode) should start with Upper
 * Case Char similar to Lexer rules.
 */
public abstract class JavaScriptBaseLexer extends Lexer {

    /**
     * Stores values of nested modes. By default mode is strict or defined
     * externally (useStrictDefault)
     */
    private Stack<Boolean> scopeStrictModes = new Stack<Boolean>();

    private Token lastToken = null;
    /**
     * Default value of strict mode Can be defined externally by
     * setUseStrictDefault
     */
    private boolean useStrictDefault = false;
    /**
     * Current value of strict mode Can be defined during parsing, see
     * StringFunctions.js and StringGlobal.js samples
     */
     ...
     * token in case it resides on the default channel. This recorded token is
     * used to determine when the lexer could possibly match a regex literal.
     * Also changes scopeStrictModes stack if tokenize special string 'use
     * strict';
     *
